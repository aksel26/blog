...
const SearchDialog: React.FC<SearchDialogProps> = ({ isOpen, onClose }) => {
  ...
  const getThumbnailUrl = (post: SearchPost): string | undefined => {
    ...
    // 상대 경로인 경우 매칭
    if (thumbnail && post.parent?.relativeDirectory) {
      const thumbnailFileName = thumbnail.replace("./", "");
      const matchedFile = data.allFile.nodes.find(
        (file: any) => file.relativeDirectory === post.parent!.relativeDirectory && file.relativePath.endsWith(thumbnailFileName)
      );
            ...
            <div className="p-6 border-b" style={{ borderColor: "var(--border-color)" }}>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3">
                  <svg className="w-5 h-5" fill="none" stroke="var(--text-tertiary)" viewBox="0 0 24 24">
                  ...
                  {filteredPosts.length > 0 ? (
                    <div ref={resultsRef} className="flex flex-col space-y-3">
                      {filteredPosts.map((post, index) => (
                        <Link key={post.fields.slug} to={post.fields.slug} onClick={onClose} style={{ textDecoration: "none" }}>
                          <div
                            className={`p-4 rounded-lg cursor-pointer transition-all duration-200`}
                            style={{
                              backgroundColor: index === selectedIndex ? "var(--accent-blue-light)" : "var(--bg-secondary)",
                            }}
                            ...
                            <div className="flex items-start space-x-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center space-x-2 mb-1">
                                  ...
                                  >
                                    {post.frontmatter.category === "기술" ? "DevLog" : "LifeLog"}
                                  </span>
                                </div>
                              ...
                              {index === selectedIndex && (
                                <div className="flex-shrink-0 mt-1">
                                  <svg className="w-4 h-4" fill="none" stroke="var(--accent-blue)" viewBox="0 0 24 24">
                    ...
                    <div className="text-center py-10">
                      <svg className="w-12 h-12 mx-auto mb-4 opacity-50" fill="none" stroke="var(--text-tertiary)" viewBox="0 0 24 24">
                        <path
                          strokeLinecap="round"
